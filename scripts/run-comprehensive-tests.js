...
async function main() {
  ...
  const perfResult = runCommand('npm run lighthouse', 'Lighthouse performance audit');
  if (perfResult.success) {
    ...
    console.log('⚠️ Lighthouse tests skipped (may require server running)');
  }
  ...
  if (testResults.coverage) {
    ...
    try {
      ...
      Object.values(testResults.coverage).forEach(file => {
        if (file && file.s) {
      ...
      if (totalLines > 0) {
        ...
        console.log(`📈 Overall Coverage: ${coveragePercent}% (${coveredLines}/${totalLines} lines)`);
      }
    } catch (err) {
